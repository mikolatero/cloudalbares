...
\begin{itemize}
\item Decidimos probar StackOps porque decia que era version de OpenStack con una facil configuracion y un rapido despliege
\pause \bigskip
\item Este sistema operativo se basa en una configuracion 'on-live' que te genera la web de stackops con un asistente paso a paso para la configuracion de openstack. Una vez instalado el SO, te pide que vayas a una pagina web y tras un pequeño registro de usuario, te muestra el asistente para poder configurar al gusto el sistema.
\pause \bigskip
\item Lo probamos para ver como funcionaria OpenStack bien montado, aun asi seguian habiendo fallos en la interface visual y varios problemas que tambien estaban en OpenStack.
\end{itemize}
\end{frame}
...
\begin{frame}\frametitle{oVirt (Abril 2013 - Junio 2013}
\begin{itemize}
\item Hasta aqui bien, el problema vino al apagar los nodos y el controlador. Cuando volvimos a arrancar he intentar iniciar las maquinas, el almacenamiento no se activaba correctamente impidiendo acceder a los datos de las maquinas virtuales.
\pause \bigskip
...
\begin{itemize}
\item Aunque conocimos de su existencia en los comienzos del proyecto, Ha sido ahora cuando lo hemos utilizado y nos hemos dado cuenta de que tambien falla.
\pause \bigskip
\item DevStack es un script de facil instalacion de OpenStack. Se puede usar sobre Ubuntu 12.04 y Fedora 16 (o posteriores)
\pause \bigskip
\item Tras clonar su repositorio de GitHub donde se encuentra su script y configurar un par de ficheros de ajustes, puedes instalar OpenStack.
\pause \bigskip
...
\pause \bigskip
\item Tras una correcta instalacion, levantamos 20 maquinas para probar que tal funcionaba. Las creaba, las eliminaba correctamente... pero a la hora de reiniciar... dejó de funcionar.
...
\end{frame}
\subsection{Red}
\begin{frame}\frametitle{Elementos de Red}
\begin{itemize}
\item En nuestro caso no tenemos cableado de fibra optica, en su lugar tendremos cableado CAT6.
\pause \bigskip
\item Tambien dispondremos de 2 switchs de 1Gbps
\pause \bigskip
\item Estos switchs tambien tienen la tecnologia para poder realizar distintas vlan, trabajar con JumboFrames y realizar Bonding entre distintas interfaces
\end{itemize}
\end{frame}
\subsection{Energia}
...
\begin{frame}\frametitle{Como se conectan los clientes con una maquina virtual}
\begin{itemize}
\item Cada una de las plataformas, tiene varias opciones para cuando un cliente quiere conectarse a la maquina virtual, siendo para cada sistema los siguientes protocolos o herramientas.
\pause \bigskip
\begin{itemize}
\item OpenNebula -- VNC
\item OpenStack -- VNC,SPICE
\item oVirt -- SPICE
\item Windows Server 2012 -- RDP
\end{itemize}
\end{itemize}
...
Este proyecto es ambicioso y tiene muy buen futuro, pero aun falta mucho desarrollo por parte de los desarrolladores de estos sistemas.

Es muy bueno que existan este tipo de proyectos ya que el mundo de la informatica esta creciendo a pasos agigantados y este proyecto que hace muy poco tiempo nacio ahora esta creciendo a un ritmo muy rapido pero aun queda una fase de maduracion del producto ya que aun hay muchos fallos en opciones basicas de funcionamiento.

Espero que con este proyecto sea un poco mas facil ver en que consisten estos sistemas de virtualizacion y ver un poco los distintos sistemas que hay y los errores que han ido surgiendo.

Gracias por haber leido este documento!! aunque esta incompleto ya que con este tema tendriamos para hacer varias presentaciones. Este es solo el principio.


PD. Este documentos no se hubiera podido hacer sin TiRoRi.
